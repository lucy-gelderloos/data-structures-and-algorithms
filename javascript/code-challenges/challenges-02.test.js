...
const addQuestion = (arr) => {
  let questionsArr = arr.map((question) => `${question}?`);
  return questionsArr;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for(let i = 0; i < arr.length; i++){
    forTwoArr.push(Math.pow(2,arr[i]));
  }
  return forTwoArr;
};
...
const forEachTwoToThe = (arr) => {
  let forEachTwoArr = [];
  arr.forEach(element => forEachTwoArr.push(Math.pow(2,element)));
...
const mapTwoToThe = (arr) => {
  let mapTwoArr = arr.map(element => Math.pow(2,element));
  return mapTwoArr;
};
...
const charCode = (arr) => {
  let charCodeArr = arr.map(letter => letter.charCodeAt(0));
  return charCodeArr;
};
...
const extractAbilities = (arr) => {
  let abilitiesArr = arr.map(element => element.ability.name);
  return abilitiesArr;
};
...
const extractStats = (arr) => {
  ...
  arr.map(element => {
    ...
    stat.total = element.effort + element.baseStat;
    statsArr.push(stat);
  });
  return statsArr;
};
