...
const toLastNames = people => {
  let namesArr = people.map(el => `${el.firstName} ${el.lastName}`);
  return namesArr;
};
...
const addValues = (arr) => {
  let sum = arr.reduce( function(startAt,el) {
    startAt = startAt + el;
    return startAt;
  }, 0);
  return sum;
...
const addPurchases = (arr) => {
  let totalPrice = arr.reduce( function(startAt,el) {
    startAt = startAt + el.purchasePrice;
    return startAt;
  }, 0);
  return totalPrice;
};
...
const countNumberOfElements = (arr) => {
  let numElements = arr.reduce (function(startAt) {
    startAt = startAt + 1;
    return startAt;
  }, 0);
  return numElements;
...
const returnNames = (arr) => {
  let namesArr = arr.reduce( function(charName,el){
    charName.push(el.name);
    return charName;
  }, []);
  return namesArr;
};
...
const countNumberOfChildren = (arr) => {
  let totalChildren = arr.reduce( function(acc,el) {
...
const calculateAverage = (arr) => {
  let sumObj = arr.reduce( function(track,el){
    track.count = track.count + 1;
    track.sum = track.sum + el;
    return track;
  }, {count:0, sum:0});
  let avgValue = sumObj.sum / sumObj.count;
  return avgValue;
};
