...
const isCapitalized = (str) => {
  let regex = /\b[A-Z].*?\b/g;
  // . means only one letter, then * is greedy, then question mark does something
  if(str.match(regex)){
...
const citiesAtoJ = (arr) => {
  let regex = /^[A-J]/g;
  // ^ tells it to look at the beginning of the string
  let citiesArr = [];
  citiesArr = arr.filter(el => el.match(regex));
  return citiesArr;
};
