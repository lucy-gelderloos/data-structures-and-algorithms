...
const typeNum = (arr) => {
  let numArr = arr.filter(el => Number.isInteger(el));
  return numArr;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let highStatsArr = arr.filter(el => el.baseStat > minBaseStat);
  return highStatsArr;
};
...
const getStatName = (arr, minBaseStat) => {
  let highStatsNameArr = [];
  arr.filter(function(el){
    if(el.baseStat > minBaseStat) {
      highStatsNameArr.push(el.stat.name);
    }
  });
  return highStatsNameArr;
};
...
const getCharactersWithoutChildren = (arr) => {
  let noChildrenArr = arr.filter(el => el.children === undefined);
  return noChildrenArr;
};
...
const evenOddNumericValues = (arr) => {
  let numbersArr = arr.filter(el => Number.isInteger(el));
  let evenOddNumbersArr = [];
  numbersArr.map(el => {
    if(el % 2 === 0) {
      evenOddNumbersArr.push('even');
    }
    else {
      evenOddNumbersArr.push('odd');
    }
  });
  return evenOddNumbersArr;
};
