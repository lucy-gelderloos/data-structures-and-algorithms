...
const toTitleCase = (arr) => {
  ...
  arr.map(el => {
    if(el.match(regex)){
      ...
      let titleString = `${match[0].toUpperCase()}${el.slice(1)}`;
...
let biggerThanLuke = (arr) => {
  let luke = arr.filter(el => el.name === 'Luke Skywalker');
  let lukesMass = luke[0].mass;
  let bigString = '';
  arr.forEach(el => {
    if (parseInt(el.mass) > parseInt(lukesMass)) {
      if (bigString.length === 0) {
        bigString = `${el.name}`;
      }
      else {
        bigString = `${bigString} - ${el.name}`;
      }
      return bigString;
    }
  });
