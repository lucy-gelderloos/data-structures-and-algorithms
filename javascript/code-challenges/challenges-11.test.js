...
function transformToLis(obj){
  ...
  keysArr.forEach(el =>
    listArr.push(`<li>${keysArr[keysArr.indexOf(el)]}: ${valuesArr[keysArr.indexOf(el)]}</li>`)
  );
  return listArr;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.map(inputEl => {
    let subArr = [];
    inputEl.map(mapEl => {
      if(typeof(mapEl) === 'number' && mapEl %5 === 0){
        subArr.push(Math.pow(2,mapEl));
      }
      return subArr;
    });
    output.push(subArr);
...
let findMaleAndFemale = (data) => {
  ...
  let nameString = '';
  data.filter(el => {
    if(el.gender === 'male' || el.gender === 'female') {
      namesArr.push(el.name);
    }
    return namesArr;
  });
  namesArr.map(el =>{
    nameString = `${nameString}${el}`;
    if(namesArr.indexOf(el) < namesArr.length - 1) {
      nameString = `${nameString} and `;
    }
    return nameString;
  }
  );
...
let findShortest = (data) => {
  let height = 0;
  data.map(el => {
    if(height === 0 || height - el.height > 0 ){
  ...
  let shortestObj = data.filter(el => el.height === height);
  console.log(shortestObj);
  return shortestObj[0].name;
};
